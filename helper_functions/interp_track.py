...
def interp_track(track: np.ndarray,
    ...
    plt.plot(track_cl[:, 0], track_cl[:, 1])
    plt.grid()
    plt.xlabel("x")
    plt.ylabel("y")
    ...
    # Use dists_cum_cl as the monotonically increasing x-axis, each dimension of track_cl as the y-axis, 
    # and use equidistantly growing dists_interp_cl as the one-dimensional array to be interpolated for interpolation.
    ...
    plt.plot(track_interp_cl[:, 0], track_interp_cl[:, 1])
    plt.grid()
    plt.xlabel("x")
    plt.ylabel("y")
